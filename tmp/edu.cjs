...
var education_content_exports = {};
__export(education_content_exports, {
  EDU_SEARCH_INDEX: () => EDU_SEARCH_INDEX,
  EDU_TOPICS: () => EDU_TOPICS,
  readingResources: () => readingResources,
  searchEducation: () => searchEducation,
  videoLessons: () => videoLessons
});
module.exports = __toCommonJS(education_content_exports);
var EDU_TOPICS = [
...
var topicName = (id) => EDU_TOPICS.find((t) => t.id === id)?.name ?? id;
var EDU_SEARCH_INDEX = [
  ...Object.entries(videoLessons).flatMap(
    ([topic, items]) => items.map((it, index) => ({ topic, topicName: topicName(topic), type: "video", title: it.title, url: it.url, index }))
  ),
  ...Object.entries(readingResources).flatMap(
    ([topic, items]) => items.map((it, index) => ({ topic, topicName: topicName(topic), type: "article", title: it.title, url: it.url, index }))
  )
];
...
function searchEducation(query) {
  ...
  return scored.filter((x) => x.score > 0).sort((a, b) => b.score - a.score).slice(0, 24).map((x) => x.doc);
}
