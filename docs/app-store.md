...
## Release readiness — September 12, 2026

September 12 corrections were squash-merged in [PR #159](https://github.com/Big5Buda/FinnaCalc/pull/159), `5ccdc97`, following September 9 PR #158 (`2e9049e`). The production-key activation [deployment](https://vercel.com/felipe-project/finnacalc-app/C2yMRtAxckjKwhstHGYE2xTkjriW) is verified Ready, source `5ccdc97`, with current domain `app.finnacalc.com`. No App Store upload or submission has been performed. Production Supabase has the Apple ownership, two-bank limit, and 30-day transcript retention migrations. Verification confirmed Apple table RLS and service-role-only access to all three functions. The first automatic retention run succeeded September 10 at 04:00 UTC; a September 12 read-only check found the latest successful run at 21:00 UTC. An earlier manual invocation removed zero expired rows.
...
The native app's separate readiness document tracks remaining submission requirements. Plaid resolved the authorized [vendor report](https://github.com/plaid/plaid-link-ios/issues/87) in its official 6.5.1 release; the native dependency and archive checker now use that correction. Native [PR #349](https://github.com/dreamingofu/FinnaCalcIOS/pull/349) is merged. The final signed archive, manifest checks and local App Store IPA export pass, including strict app/widget signatures and App Store profiles. Real device/Sandbox tests remain unverified: the paired phone is unreachable and the owner has no cable available. The new U.S. Sandbox tester is created and verified in App Store Connect; it has not yet completed a purchase test.

September 12 backend corrections preserve retryable SnapTrade deletion credentials, recognize the actual SDK's top-level error status, and persist accepted vendor deletion before later provider/account deletion steps. Apple's retryable certificate-verification failures return HTTP 503 while invalid proofs remain HTTP 400. Public About, Terms and tax copy now matches implemented scope, including the hard two-bank limit and separate native/web tax years. These corrections are deployed; public pages returned HTTP 200 and their corrected copy was checked. The full backend suite passes 401 tests and both production workspace builds pass. Vercel remains on Hobby, and the owner has not yet completed the required commercial-plan upgrade. Both active projects' PR preview checks passed; a pre-existing check for a separate project in another team failed and is inaccessible to the signed-in account, so its cause remains unverified.

The owner-approved SnapTrade ownership migration is installed in production. Before installation, a simulator-triggered `/brokerages` request was verified HTTP 200 in the legacy key's request log at **2026-09-12 21:42:22 UTC**, establishing the deployed original Client ID as `FINNACALC-TEST-PKUEY`. Post-installation verification found all **3** existing rows under that ID, **0** missing owners, RLS and the guard trigger enabled, and deletion-RPC execution available to service_role but unavailable to anon/authenticated. The migration also passed **22** isolated PostgreSQL checks. After compatible deployment and legacy-account reads passed, Production-only `SNAPTRADE_USE_PRODUCTION_KEY=true` was activated and redeployed. The new key's catalog read and both keys' test-webhook deliveries returned HTTP 200; legacy-account reads still succeeded. Final counts are **4 legacy rows** (the original three plus one empty QA mapping), **0 production rows**, **0 missing owners**. No brokerage authorization or trade was performed. See the [cutover evidence, delivery limits and rollback guide](snaptrade-key-cutover.md).

## View-only brokerage — September 24, 2026

At the owner's direction, FinnaCalc no longer places, previews or cancels brokerage orders on the web or through its API. The SnapTrade order placement (`/api/snaptrade/trade/place`), order impact (`/api/snaptrade/trade/impact`), cancellation (`/api/snaptrade/orders/cancel`) and order-ticket quote (`/api/snaptrade/quote`) routes are removed; installed builds that still call them receive 404. Read-only positions, balances and order history (`GET /api/snaptrade/orders`) remain. The web trade ticket is gone and `/investing/trade/:symbol` redirects to `/investing/stocks/:symbol`.

`POST /api/snaptrade/connect` always requests `connectionType: "read"` and ignores any client `access` field. `reconnect` is accepted only for an owned connection whose SnapTrade type is exactly `read`; a `trade` or missing/unknown type returns `409` with `code: connection_not_read_only`, asking the user to disconnect and link again. Nothing disconnects accounts automatically, and no copy claims that a legacy trading grant was revoked at the brokerage. The portfolio page flags such links with an explicit disconnect-and-relink action.

This change is committed only; it is not deployed. Production still serves the earlier routes until a deliberate deployment.
...
ASC currently points both Production and Sandbox notifications to `https://finnacalc.com/api/apple/notifications`. The backend implements `POST /api/apple/notifications` for the standard `{ "signedPayload": "Apple JWS" }` body and is deployed in `2e9049e`. The existing marketing `/api/*` rewrite forwards that path to the app backend. Verify the exact configured host with a real Apple test notification; the direct application URL is `https://app.finnacalc.com/api/apple/notifications` if avoiding a host redirect is necessary. Confirm ASC uses version 2. Read-only method checks confirm route presence but do not prove a successful delivery from Apple's live notification service.
...
Existing active, unexpired Stripe subscriptions remain valid for their feature. Free `/api/chat` and quick Budget Advisor responses retain their existing access. A client-supplied tier never grants server access. Fresh SnapTrade portal creation allows one free brokerage connection and requires verified Investing Plus/Pro for another. Existing owned view-only reconnects remain available. Portfolio and refresh requests recheck the current count and reject multiple free connections with an explicit error. Connection management and viewing existing order history stay available. No brokerage is automatically deleted or arbitrarily excluded from a reported total.
